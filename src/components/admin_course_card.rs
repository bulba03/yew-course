...
#[function_component]
pub fn AdminCourseCard(props: &Props) -> Html {
    let navigator = use_navigator().unwrap();
    let id = props.id;
    let title = props.title.clone();
    let desctiption = props.description.clone();
    let image_url = props.image_url.clone();
    let onclick = Callback::from(move |_| navigator.push(&Route::CourseEdit { id: id.clone() as i32}));
    html! {
        ...
        <div class="card-footer is-centered">
                <button class="card-footer-item button has-addons is-dark" onclick ={onclick}>{"Редактировать"}</button>
